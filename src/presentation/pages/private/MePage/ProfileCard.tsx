...
import ActionMenu, {
  type ActionMenuItem,
} from "@/presentation/ui/ActionMenu.tsx";
...
const ProfileCard: FC<ProfileCardProps> = ({
  ...
  const actions: ActionMenuItem[] = [
    ...
    { label: "Cerrar sesión", onClick: onLogout, variant: "danger" },
    ...(isAdmin
      ? [{ label: "Admin", onClick: onGoAdmin } as ActionMenuItem]
      : []),
  ];
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-xl border border-gray-300 flex items-center flex-col sm:flex-row w-full
                   px-[clamp(1.25rem,4vw,2.5rem)] py-[clamp(1rem,1vw,2rem)]
    ...
    >
      ...
      <div className="flex flex-col min-w-0 flex-1 gap-[clamp(0.35rem,1vw,0.6rem)] sm:items-start items-center">
        ...
        <div className="hidden xl:flex items-center gap-[clamp(0.5rem,1.2vw,0.75rem)]">
          {actions.map((action) => (
            <Button
              key={action.label}
              label={action.label}
